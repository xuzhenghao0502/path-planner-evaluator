...
class RSPath {
 public:
  ...
  struct RSPathData {
   public:
    explicit RSPathData(const RSPathSegmentType* type = RS_path_segment_type[0],
                        double t = std::numeric_limits<double>::max(), double u = 0.0, double v = 0.0, double w = 0.0,
                        double x = 0.0)
        : type_(type) {
      length_[0] = t;
      length_[1] = u;
      ...
      length_[3] = w;
      length_[4] = x;
      total_length_ = std::fabs(length_[0]) + std::fabs(length_[1]) + std::fabs(length_[2]) + std::fabs(length_[3]) +
                      std::fabs(length_[4]);
    }
   ...
   */
  std::vector<Eigen::Vector3d> GetRSPath(const Eigen::Vector3d& start_state, const Eigen::Vector3d& goal_state,
                                         double step_size, double& length);
  std::vector<vector<PathPt>> GetRSPath(const Eigen::Vector3d& start_state, const Eigen::Vector3d& goal_state,
                                        const double step_size, double& length, int& num, bool s_enable = true);

  RSPathData GetRSPath(double x_0, double y_0, double yaw_0, double x_1, double y_1, double yaw_1);
