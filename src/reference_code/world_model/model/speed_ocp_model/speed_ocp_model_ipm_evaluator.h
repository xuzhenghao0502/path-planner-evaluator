...
class SpeedOCPModelIpmEvaluator : public IpmEvaluator {
 public:
  SpeedOCPModelIpmEvaluator() : IpmEvaluator(3, 8, 15, 0, 0) {}
  virtual ~SpeedOCPModelIpmEvaluator() = default;
  ...
  virtual void updateLX(const Eigen::VectorXd& l_x, const Eigen::MatrixXd& Fx, const Eigen::MatrixXd& g_x,
    ...
    sl_x(0) = Fx(0, 0) * p(0) + g_x(0, 0) * lambda(0) + g_x(1, 0) * lambda(1) + g_x(14, 0) * lambda(14) +
              g_x(8, 0) * lambda(8) + g_x(9, 0) * lambda(9) + l_x(0);
  ...
  virtual void updateQH(const Eigen::MatrixXd& l_xx, const Eigen::MatrixXd& l_xu, const Eigen::MatrixXd& l_uu,
    ...
    const double internal_0 = Sigma_(14) * g_x(14, 0) * g_x(14, 1);

    // Evaluation of Matrix Q_xx_
    Q_xx_(0, 0) = Fx_0_0_p2 * V_xx_prev(0, 0) + Sigma_(0) * g_x_0_0_p2 + Sigma_(1) * g_x_1_0_p2 +
                  Sigma_(14) * g_x_14_0_p2 + Sigma_(8) * g_x_8_0_p2 + Sigma_(9) * g_x_9_0_p2 + l_xx(0, 0);
    Q_xx_(0, 1) =
        Fx(0, 0) * Fx(0, 1) * V_xx_prev(0, 0) + Fx(0, 0) * Fx(1, 1) * V_xx_prev(0, 1) + internal_0 + l_xx(0, 1);
    Q_xx_(0, 2) = Fx(0, 0) * Fx(1, 2) * V_xx_prev(0, 1) + Fx(0, 0) * Fx(2, 2) * V_xx_prev(0, 2);
    Q_xx_(1, 0) = Fx(0, 0) * (Fx(0, 1) * V_xx_prev(0, 0) + Fx(1, 1) * V_xx_prev(1, 0)) + internal_0 + l_xx(1, 0);
  ...
  virtual void updateQG(const Eigen::VectorXd& sl_x, const Eigen::VectorXd& sl_u, const Eigen::MatrixXd& g_x,
    ...
    Q_x_(0) =
        ...
        V_(0) * g_x(0, 0) + V_(1) * g_x(1, 0) + V_(14) * g_x(14, 0) + V_(8) * g_x(8, 0) + V_(9) * g_x(9, 0) + sl_x(0);
    Q_x_(1) =
  ...
  virtual void updateVxx(const Eigen::MatrixXd& k_ux, const Eigen::MatrixXd& k_ex, const Eigen::VectorXd& v_u,
    ...
    V_xx(0, 0) = Q_xu_(0, 0) * k_ux(0, 0) + Q_xu_(0, 1) * k_ux(1, 0) + Q_xu_(0, 2) * k_ux(2, 0) +
                 Q_xu_(0, 7) * k_ux(7, 0) + Q_xx_(0, 0);
    V_xx(0, 1) = Q_xu_(0, 0) * k_ux(0, 1) + Q_xu_(0, 7) * k_ux(7, 1) + Q_xx_(0, 1);
    V_xx(0, 2) = Q_xu_(0, 0) * k_ux(0, 2) + Q_xx_(0, 2);
    V_xx(1, 0) = Q_xu_(1, 0) * k_ux(0, 0) + Q_xu_(1, 7) * k_ux(7, 0) + Q_xx_(0, 1);
    ...
    V_x(0) = Q_x_(0) + Q_xu_(0, 0) * v_u(0) + Q_xu_(0, 1) * v_u(1) + Q_xu_(0, 2) * v_u(2) + Q_xu_(0, 7) * v_u(7);
...
template <>
class SpeedOCPModelIpmEvaluator<StageType::INITIAL, OcpConfig::FORWARD_EULER> : public IpmEvaluator {
 public:
  SpeedOCPModelIpmEvaluator() : IpmEvaluator(3, 8, 2, 0, 3) {}
  virtual ~SpeedOCPModelIpmEvaluator() = default;
...
template <>
class SpeedOCPModelIpmEvaluator<StageType::NORMINAL, OcpConfig::ERK4> : public IpmEvaluator {
 public:
  SpeedOCPModelIpmEvaluator() : IpmEvaluator(3, 8, 15, 0, 0) {}
  virtual ~SpeedOCPModelIpmEvaluator() = default;
...
template <>
class SpeedOCPModelIpmEvaluator<StageType::INITIAL, OcpConfig::ERK4> : public IpmEvaluator {
 public:
  SpeedOCPModelIpmEvaluator() : IpmEvaluator(3, 8, 2, 0, 3) {}
  virtual ~SpeedOCPModelIpmEvaluator() = default;
...
class SpeedOCPModelIpmEvaluatorTerminal : public IpmEvaluatorTerminal {
 public:
  SpeedOCPModelIpmEvaluatorTerminal() : IpmEvaluatorTerminal(3, 8, 6, 0) {}
  virtual ~SpeedOCPModelIpmEvaluatorTerminal() = default;
