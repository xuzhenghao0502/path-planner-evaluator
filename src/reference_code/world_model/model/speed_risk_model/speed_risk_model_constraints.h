...
class SpeedRiskModelConstraints : public Constraints {
 public:
  SpeedRiskModelConstraints() : Constraints(3, 8, 6, 2, 7) {
    ...
    exprs_[10] = "-VSoftUpperBound - df*s + residual - slack_v_upper + v <= 0";
  ...
  virtual ~SpeedRiskModelConstraints() = default;
  ...
  virtual void updateEvaluate(const Eigen::VectorXd& states, const Eigen::VectorXd& ctrls,
    ...
    const auto& residual = params(31);
    const auto& df = params(32);
    ...
    values(10) = -VSoftUpperBound - df * s + residual - slack_v_upper + v;
  ...
  virtual void updateDeltaEvaluation(const Eigen::MatrixXd& g_x, const Eigen::MatrixXd& g_u, const Eigen::VectorXd& dx,
    ...
    dg(10) = du(3) * g_u(10, 3) + dx(0) * g_x(10, 0) + dx(1) * g_x(10, 1);
...
template <>
class SpeedRiskModelConstraints<StageType::INITIAL> : public Constraints {
 public:
  SpeedRiskModelConstraints() : Constraints(3, 8, 0, 2, 0) {
    // Evaluation of Matrix jacobian_.ju
    jacobian_.ju(0, 0) = -1;
    jacobian_.ju(1, 0) = 1;
...
template <>
class SpeedRiskModelConstraints<StageType::TERMINAL> : public Constraints {
 public:
  SpeedRiskModelConstraints() : Constraints(3, 8, 6, 0, 0) {
    // Evaluation of Matrix jacobian_.jx
    jacobian_.jx(0, 0) = -1;
    jacobian_.jx(1, 0) = 1;
    jacobian_.jx(2, 1) = -1;
...
template <StageType Ttype>
class SpeedRiskModelStateOnlyEqualities : public Constraints {
 public:
  SpeedRiskModelStateOnlyEqualities() : Constraints(3, 8, 0, 0, 0) {}

  virtual ~SpeedRiskModelStateOnlyEqualities() = default;

  virtual Type type() const override { return Type::STATE_ONLY_EQUALITY; }
};

template <>
class SpeedRiskModelStateOnlyEqualities<StageType::INITIAL> : public Constraints {
 public:
  SpeedRiskModelStateOnlyEqualities() : Constraints(3, 8, 3, 0, 0) {
...
template <>
class SpeedRiskModelStateOnlyEqualities<StageType::TERMINAL> : public Constraints {
 public:
  SpeedRiskModelStateOnlyEqualities() : Constraints(3, 8, 0, 0, 0) {}
