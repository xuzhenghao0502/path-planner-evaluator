...
gen_dir = solver_path + "/model/speed_risk_model"
if len(sys.argv) > 1:
    gen_dir = sys.argv[1]

ocp_model = OptimalControlProblem("SpeedRiskModel")
...
residual, df = ocp_model.add("param", residual=0.0, df=0.0)

# define cost
cost_x = SWeight * (s - SRef + K * v) ** 2 + VWeight * (v - VRef) ** 2 + AWeight * a**2
cost_u = JWeight * j**2
cost_slack = (
...
ocp_model.subject(v - (df * s + VSoftUpperBound - residual) - slack_v_upper <= 0)
